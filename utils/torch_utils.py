...
def torch_distributed_zero_first(local_rank: int):
    ...
    # Patch 2026-07-08: dist.barrier(device_ids=...) is NCCL-only; CPU DDP (gloo) needs a plain barrier.
    use_device_ids = dist.is_available() and dist.is_initialized() and dist.get_backend() == "nccl"
    if local_rank not in [-1, 0]:
        dist.barrier(device_ids=[local_rank]) if use_device_ids else dist.barrier()
    yield
    if local_rank == 0:
        dist.barrier(device_ids=[0]) if use_device_ids else dist.barrier()
